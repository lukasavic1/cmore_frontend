...
function Column({ title, statusKey, tasks, onEdit, onDropTask }) {
  return (
    <div className="flex-1 min-w-[260px] bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-[var(--shadow-sm)] overflow-hidden">
      ...
      >
        {tasks.length ? (
          ...
          <div className="flex-1 flex items-center justify-center rounded-xl border border-dashed border-[var(--border)] bg-[var(--surface-2)] text-xs text-[var(--muted)] py-10">
            Drop tasks here
          </div>
        )}
      </div>
...
export default function KanbanBoard({ tasks, isLoading, isError, error, onEdit, onMoveTask }) {
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        {COLUMNS.map((c) => (
          <div key={c.key} className="h-[320px] bg-[var(--surface)] border border-[var(--border)] rounded-2xl animate-pulse" />
        ))}
      </div>
    )
  }
      ...
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        {status === 401 && (
          <p className="text-xs text-[var(--muted)] mt-2">
            This usually means your `auth_token` is missing/expired. Try signing out and signing in again.
          </p>
        )}
      </div>
    )
