 ...
 *
 * Shows the marketing-style landing page and sign-in entry for the
 * application.
 */
...
const LANDING_FEATURES = [
  {
    ...
    lead:
      "Tasks live on a three-column board — To do, In" +
      "progress, and Done. Move work forward the way your team" +
      "already thinks about delivery, without leaving the" +
  ...
  },
  {
    ...
    lead:
      "ESG and sustainability work competes with everything" +
      "else on the calendar. This demo keeps priority and" +
      "deadlines on the surface so the next action is never a" +
      "guessing game.",
    points: [
  ...
  },
  {
    ...
    lead:
      "Once the board fills up, you need a fast way to narrow" +
      "in. The app combines live search, status filters, and a" +
      "statistics strip so you can answer “where are we?” in" +
      "seconds.",
    points: [
...
function KanbanMock() {
  const col = (title, items, tint) => (
    <div
      className={`flex-1 min-w-[140px] rounded-xl border border-[var(--border)]
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 w-full max-w-xl mx-auto">
      {col(
        "To do",
        ["Carbon report draft", "Vendor survey"],
        "var(--muted-2)",
      )}
...
function PriorityMock() {
  const badges = [
    ...
    },
    {
      label: "Medium",
      cls:
        "bg-[color-mix(in_srgb,var(--warning)_18%,white)]" +
        "text-[var(--warning)]" +
        "border-[color-mix(in_srgb,var(--warning)_40%,var(--border))]",
    },
    {
      label: "Low",
      cls: "bg-[var(--surface-2)] text-[var(--muted)] border-[var(--border)]",
    },
  ];
  return (
    <div
      className={`w-full max-w-md mx-auto rounded-2xl border
      border-[var(--border)] bg-[var(--surface)] p-5
      shadow-[var(--shadow-sm)] space-y-4
    `}
    >
      ...
      <div className="rounded-xl border border-[var(--border)] p-3 text-sm">
        <div className="flex justify-between items-start gap-3">
          ...
          <span
            className={`text-xs text-[var(--danger)] font-medium whitespace-nowrap
          `}
          >
            Overdue
          </span>
  ...
  return (
    <div className="w-full max-w-md mx-auto space-y-3">
        ...
        {[
          ...
          <div
            ...
            className={`rounded-xl border border-[var(--border)] bg-[var(--surface)]
              px-3 py-2 text-center min-w-[4.5rem] shadow-[var(--shadow-sm)]
            `}
          >
      ...
      <div
        ...
        shadow-[var(--shadow-sm)] flex items-center gap-2
      `}
      >
        <svg
        ...
        >
          <path
            ...
            d={
              "M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0" +
              "0010.607 10.607z"
            }
          />
        </svg>
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-center gap-3 px-4 py-2.5
        ...
        text-[var(--text)] text-sm shadow-[var(--shadow-sm)]
      `}
    >
      <svg
        width="18"
      ...
      >
        <path
          fill="#4285F4"
          d={
            "M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209" +
        ...
        <path
          fill="#34A853"
          d={
            "M9 18c2.43 0 4.467-.806" +
            "5.956-2.184l-2.908-2.258c-.806.54-1.837.86-3.048.86-2.344" +
            "0-4.328-1.584-5.036-3.711H.957v2.332C2.438 15.983 5.482 18 9" +
            "18z"
          }
        />
        <path
          ...
          d={
            "M3.964" +
            "10.707c-.18-.54-.282-1.117-.282-1.707s.102-1.167.282-1.707V4.961H.957C.347" +
            "6.175 0 7.55 0 9s.348 2.825.957 4.039l3.007-2.332z"
          }
        />
        <path
          fill="#EA4335"
          d={
            "M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891" +
            "11.426 0 9 0 5.482 0 2.438 2.017.957 4.961L3.964 7.293C4.672" +
...
function Field({
  ...
  const id =
    name ?? `auth-${String(label).replace(/\s+/g, "-").toLowerCase()}`;
  return (
    <div>
      <label
        htmlFor={id}
      ...
      />
      {error && (
        <p id={`${id}-err`} className="mt-1.5 text-xs text-[var(--danger)]">
          {error}
        </p>
      )}
    </div>
...
function AuthCard() {
  ...
  async function handleSignUp(e) {
    ...
    if (!name.trim()) errs.name = "Name is required.";
      ...
      errs.name = "Name must be 128 characters or fewer.";
    if (!emailTrim) errs.email = "Email is required.";
    else if (!isValidEmail(emailTrim))
      errs.email = "Enter a valid email address.";
    if (!password) errs.password = "Password is required.";
  ...
  return (
    <div
      className={`w-full max-w-sm bg-[var(--surface)] border
      border-[var(--border)] rounded-2xl shadow-[var(--shadow)] p-6
    `}
    >
      <div
        className={`flex rounded-xl border border-[var(--border)] overflow-hidden
        mb-5 bg-[var(--surface-2)] shadow-[var(--shadow-sm)]
      `}
      >
        {[
          ["google", "Google"],
      ...
      {error && (
        <div
          className={`mb-4 px-3 py-2.5 bg-[var(--danger-bg)] border
      ...
      {tab === "signin" && (
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-2.5 bg-[var(--primary)] text-white text-sm
              font-medium rounded-xl hover:bg-[var(--primary-hover)]
              disabled:opacity-50 disabled:cursor-not-allowed
              transition-colors mt-1 shadow-sm
            `}
          >
...
function friendlyError(err) {
  const map = {
    ...
    "auth/unauthorized-domain":
      "This domain is not authorised in Firebase Console.",
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      <header
        ...
        backdrop-blur-md
        supports-[backdrop-filter]:bg-[color-mix(in_srgb,var(--surface)_85%,transparent)]
      `}
      >
        <div
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            <div
              ...
              bg-[color-mix(in_srgb,var(--primary)_10%,white)] border
              border-[color-mix(in_srgb,var(--primary)_18%,white)] flex
              items-center justify-center flex-shrink-0
            `}
            >
              <svg
                className="w-4.5 h-4.5 text-[var(--primary)]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={1.5}
            ...
            <span
              className={`font-semibold text-[var(--text)] text-base tracking-tight
              truncate
            `}
            >
              C-More Task Tracker
          ...
          <a
            ...
            className={`text-xs font-medium text-[var(--primary)]
              hover:text-[var(--primary-hover)] hidden sm:inline
              transition-colors
            `}
          >
      ...
      <section
        className={`max-w-6xl mx-auto px-6 pt-16 pb-12 lg:pt-24 lg:pb-20
        min-h-[calc(100dvh-4.5rem)] flex flex-col justify-center
      `}
      >
        <div
        ...
        >
          <div className="flex-1 text-center lg:text-left w-full">
            <div
              ...
              bg-[color-mix(in_srgb,var(--primary)_10%,white)] border
              border-[color-mix(in_srgb,var(--primary)_18%,white)]
              text-[var(--primary)] text-xs font-medium px-3.5 py-1.5
              rounded-full mb-6
            `}
            >
              <span
            ...
            <h1
              ...
              text-[var(--text)] tracking-tight mb-6 leading-[1.08]
            `}
            >
            ...
            <p
              className={`text-base sm:text-lg text-[var(--muted)] max-w-xl mx-auto
              lg:mx-0 leading-relaxed mb-4
            `}
            >
              This is a deliberately small demo: a task tracker themed around
              ESG and sustainability work. It exists to show a credible product
              shell — sign-in, a Kanban board, and everyday task mechanics —
              wired to a real backend and auth, not to replace your production
              tooling.
            </p>
            <p
              className={`text-sm text-[var(--muted-2)] max-w-xl mx-auto lg:mx-0
              leading-relaxed mb-8
            `}
            >
              Sign in below to try it. The next sections walk through three
              things the UI is built to highlight, one at a time.
            </p>
            <div
            ...
            >
              <a
                ...
                className={`inline-flex items-center justify-center text-sm font-medium
                  ...
                  bg-[color-mix(in_srgb,var(--primary)_6%,white)]
                  hover:bg-[color-mix(in_srgb,var(--primary)_10%,white)] px-4
              ...
              <span className="text-xs text-[var(--muted)]">
                Laravel API · React · Firebase Auth
              </span>
            </div>
          </div>
          <div
      ...
      {LANDING_FEATURES.map((f, index) => {
        ...
        return (
          ...
          >
            <div
              className={`max-w-6xl mx-auto px-6 py-20 sm:py-28 lg:py-32 min-h-[85dvh]
                ...
                <div className={`space-y-6 ${isReversed ? "lg:order-2" : ""}`}>
                  ...
                  <ul className="space-y-3 max-w-prose">
                    {f.points.map((p) => (
                      <li
                        key={p}
                        className={`flex gap-3 text-sm sm:text-base text-[var(--text)]
                          leading-relaxed
                        `}
                      >
                        <span
                          className={`flex-shrink-0 w-5 h-5 rounded-full
                          bg-[color-mix(in_srgb,var(--success)_15%,white)] border
      ...
      <footer
        className={`border-t border-[var(--border)] py-10 bg-[var(--surface)]
      `}
      >
        <div
          className={`max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center
          justify-between gap-4 text-xs text-[var(--muted)] text-center
          sm:text-left
        `}
        >
          <span>
            © {new Date().getFullYear()} C-More Task Tracker — demo only
          </span>
          <a
            href="#sign-in"
            className={`text-[var(--primary)] hover:text-[var(--primary-hover)]
