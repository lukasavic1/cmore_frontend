 ...
 *
 * Composes the main authenticated app shell, page layout, and task
 * management flows.
 */
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-3">
        <svg
          className="animate-spin w-7 h-7 text-[var(--primary)]"
          fill="none"
          viewBox="0 0 24 24"
        >
          <circle
    ...
    <div className="relative">
      ...
      >
        {user.avatar ? (
          ...
          <div
            ...
            bg-[color-mix(in_srgb,var(--primary)_12%,white)] border
            border-[color-mix(in_srgb,var(--primary)_25%,white)] flex
            items-center justify-center text-[var(--primary)]
      ...
      {open && (
        <>
          ...
          <div
            className={`absolute right-0 mt-2 w-52 bg-[var(--surface)] border
            border-[var(--border)] rounded-xl shadow-[var(--shadow)] z-20
          ...
          >
            ...
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm text-[var(--danger)]
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)]">
      <div className="max-w-5xl mx-auto px-4 py-8">
        {/* Header */}
        <div
          className={`sticky top-0 z-20 -mx-4 px-4 pt-4 pb-4 mb-6 bg-[var(--bg)]
        `}
        >
          <div
            className={`flex items-center justify-between bg-[var(--surface)] border
            border-[var(--border)] rounded-2xl px-4 py-3
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 bg-[var(--primary)]
                  text-white text-sm font-medium rounded-xl
                  hover:bg-[var(--primary-hover)] transition-colors shadow-sm
                `}
              >
                <svg
                  className="w-4 h-4"
        ...
        <div className="mb-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              ...
              className={`mt-1 px-4 py-2 text-sm font-medium bg-[var(--surface)] border
                border-[var(--border)] rounded-xl hover:bg-[var(--surface-2)]
                hover:border-[var(--border-strong)] transition-colors
                shadow-[var(--shadow-sm)] text-[var(--text)]
              `}
            >
              Deeper analysis
            </button>
      ...
      {modal === "create" && (
        <TaskModal title="New task" onClose={() => setModal(null)}>
          <TaskForm
            apiFieldErrors={
              create.isError ? create.error?.response?.data?.errors : undefined
            }
      ...
      {modal?.task && (
        <TaskModal title="Edit task" onClose={() => setModal(null)}>
          <TaskForm
            initial={modal.task}
            apiFieldErrors={
              update.isError ? update.error?.response?.data?.errors : undefined
            }
            onSubmit={(payload) =>
              update.mutate({ id: modal.task.id, ...payload })
            }
            onCancel={() => setModal(null)}
